    ...
    private fun checkCurrentAssociations() {
        scope.launch {
            try {
                ...
                val gunzipAssociations = associations.filter {
                    it.applicationName.contains("Gunzip", ignoreCase = true)
                }

                _uiState.value = _uiState.value.copy(
                    currentAssociations = associations,
                    gunzipAssociations = gunzipAssociations,
                    isRegistered = gunzipAssociations.isNotEmpty()
                )

                logger.i { "Found ${gunzipAssociations.size} Gunzip associations out of ${associations.size} total" }
...
data class FileAssociationUiState(
    ...
    val gunzipAssociations: List<FileAssociation> = emptyList(),
    ...
    val registeredExtensions: List<String>
        get() = gunzipAssociations.map { it.extension }
